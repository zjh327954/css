...
async def main():
    ...
    if not final_items:
        print(f"[-] {DEFAULT_TARGETS} 无有效目标，跳过 iptest 精测环节。", flush=True)
        return

    csv_filename = f"{clean_input[:30]}.csv"
    final_csv_path = os.path.join(output_dir, csv_filename)
    ...
    iptest_cmd = [
        IPTEST_BIN,
        f"-file={tmp_ip_file}",
        f"-outfile={tmp_out_csv}",
        f"-max={IPTEST_MAX_COROUTINES}",
        f"-speedtest={IPTEST_SPEEDTEST}",
        "-tls=true"
    ]
    ...
    try:
        ...
        if res.returncode == 0 and os.path.exists(tmp_out_csv) and os.path.getsize(tmp_out_csv) > 0:
            ...
            print(f"[+] CSV 结果已成功生成并保存至: {final_csv_path}", flush=True)
        else:
        ...
        print(f"[-] 调用 iptest 异常: {e}", flush=True)
        with open(final_csv_path, "w", encoding="utf-8-sig") as f:
            f.write("IP地址,端口号,TLS\n")
            for ip, port in final_items:
                f.write(f"{ip},{port},TRUE\n")
        ...
        if os.path.exists(tmp_out_csv):
            os.remove(tmp_out_csv)

    print(f"[✓] {DEFAULT_TARGETS} 处理完毕！结果文件: {final_csv_path}\n", flush=True)
