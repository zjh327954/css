...
async def main():
    ...
    # ==================== 1. 多进程 TLS 粗筛 ====================
    print(f"\n[1/1 TLS 探测] 多进程并行并发中...", flush=True)
    ...
    print(f"[+] 第一阶段探测完成！匹配 CF 证书保留目标: {len(pass_1)} 个\n", flush=True)

    if not pass_1:
        print("[-] 无有效 IP:端口 通过测试，程序退出。", flush=True)
        return

    # 按 IP 地址数值大小排序
    final_items = sorted(pass_1, key=lambda x: (ipaddress.ip_address(x[0]), x[1]))
